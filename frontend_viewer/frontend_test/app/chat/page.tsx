              ...
              {panelChunks.length === 0 ? (
                ...
                <div className="space-y-3">
                  {panelChunks.map((chunk, index) => {
                    const pageImageUrl = chunk.metadata?.page_image_url
                    const pageNum = chunk.metadata?.page
                    const totalPages = chunk.metadata?.total_pages
                    const totalImages = chunk.metadata?.total_images
                    const totalTables = chunk.metadata?.total_tables

                    return (
                    <a
                      key={chunk.id ?? index}
                      href={`/result/${encodeURIComponent(chunk.id)}`}
                      className="block rounded-lg border border-slate-800 bg-slate-900/70 hover:bg-slate-800/70 hover:border-emerald-500/50 p-3 text-xs space-y-2 transition-all cursor-pointer"
                      onClick={(e) => {
                        // Save this chunk to sessionStorage for detail page
                        sessionStorage.setItem("searchResults", JSON.stringify([chunk]))
                      }}
                    >
                      {/* Page Image Thumbnail */}
                      {pageImageUrl && (
                        <div className="w-full bg-slate-950 rounded border border-slate-700 overflow-hidden">
                          <img
                      ...
                      <div className="flex items-center justify-between mt-1 text-[10px] text-slate-500 flex-wrap gap-1">
                        <div className="flex items-center gap-2">
                          {chunk.category && <span>{chunk.category}</span>}
                          {typeof totalImages === 'number' && totalImages > 0 && (
                            <span>🖼️ {totalImages} img</span>
                          )}
